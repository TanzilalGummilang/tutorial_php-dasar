...
function login($data_login){
	...
	$username = strtolower(htmlspecialchars($data_login["username"]));
	$password = htmlspecialchars($data_login["password"]);

	// cek username apakah ada di database
	$query_cek_username = mysqli_query($conn, "SELECT * FROM tbl_user WHERE username = '$username'");
	if ( mysqli_num_rows($query_cek_username) === 1) {
		
		// cek password
		$row = mysqli_fetch_assoc($query_cek_username);
		if ( password_verify($password, $row["password"]) or $row["password"] == true ) {
			$_SESSION["login"] = true;

			// jika checkbox remember me di tekan
			if ( isset($_POST["remember"]) ) {
				
				// bikin cookienya
				setcookie('id', $row["id"], time() + 60*2);
				setcookie('username', hash('sha256', $row['username']), time() + 60*2);
			}
